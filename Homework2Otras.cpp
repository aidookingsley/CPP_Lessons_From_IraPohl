//HOMEWORK 2 : IMPLEMENT DIJKSTRA's ALGORITHM And CALCULATE THE AVERAGE SHORTEST PATH IN A GRAPH
//REFERENCE : http://en.wikipedia.org/wiki/Dijkstras_algorithm 
...
public:
    ...
    unsigned int V() const { return v_; }                        //return number of vertices

    unsigned int E() const { return e_; }                        //return number of edges
    ...
    void add_edge(unsigned int x, unsigned int y, double weight) {            //add edge from node x to y
        if (weight < 0)
    ...
    void remove_edge(unsigned int x, unsigned int y) {                       //remove edge from node x to y
...
class ShortestPath {                                 //compute the shortest path
public:
...
public:
    Simulator(int num = 50, double density = 0.2,
        ...
        num_(num),
        den_(density),
        random_generator_(time(NULL)),                               //psuedo random generator
    ...
    double average_distance() const {                 //return the average distance of shortest path
        return avg_;
    }

protected:
    double run_simulation() {                       //generate graph simulation
